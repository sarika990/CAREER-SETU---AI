...
class ResumeScorer:
    ...
    async def score_resume(self, resume_text: str, target_role: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Cache check
        prompt_hash = gemini_service.get_prompt_hash(prompt)
        cache_key = f"gemini:resume_analysis:{prompt_hash}"
        
        if cache_manager.enabled:
            cached_result = await cache_manager.get(cache_key)
            if cached_result:
                logger.info(f"📊 Resume Analysis Cache Hit: {cache_key}")
        ...
        if ai_response:
            try:
                ...
                if cache_manager.enabled:
                    await cache_manager.set(cache_key, final_result, expire=86400)
                
                return final_result
            except Exception as e:
                logger.error(f"Error parsing Gemini response for resume: {e}")
